...
def _logo_bytes() -> Optional[bytes]:
    ...
    if raw and not raw.startswith(("http://", "https://", "data:")):
        ...
        if not candidate.is_absolute():
            app_root = (os.getenv("APP_ROOT") or str(Path(__file__).resolve().parents[1]))
            candidate = Path(app_root) / raw.lstrip("/\\")
        p = candidate
    else:
        p = Path(__file__).resolve().parents[1] / "static" / "brand" / "ecolocal-logo-transparent.png"
    try:
        if p and p.is_file():
            return p.read_bytes()
    except Exception:
        log.exception("[inbox] failed reading logo file path=%s", p)
    return None

_LOGO_BYTES = None  # Force disable embedding to avoid inline/SES quirks
_SENDER_LOGO_SRC_PUBLIC = "https://elocal.ecodia.au/static/brand/ecolocal-logo-transparent.png"
_SENDER_LOGO_SRC_CID = f"cid:{_LOGO_CID}"

def signature_inline_images() -> List[dict]:
    if _LOGO_BYTES:
        return [{"cid": _LOGO_CID, "bytes": _LOGO_BYTES}]
    return []

def _signature_html() -> str:
    logo_src = _SENDER_LOGO_SRC_CID if _LOGO_BYTES else _SENDER_LOGO_SRC_PUBLIC
    return f"""
...
def triage_and_update(message: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # (ICS is fine; we just don't want misleading copy)
    ...
    try:
        ...
        meta = {
            ...
            "semantic_context": (getattr(flow, "semantic_context", None) or []),  # 👈 add this
